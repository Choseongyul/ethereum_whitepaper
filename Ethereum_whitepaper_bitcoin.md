...
<h2>스크립팅</h2>
비트코인의 UTXO는 공개키뿐 아니라 간단한 스택 기반 프로그래밍 언어로 구현되는 스크립트에 의해서도 소유되고 있다.</br>
공개 키 소유권 메커니즘 또한 스크릡트에 의해 구현되어 있는데, 타원곡선의 서명을 입력으로 하고, 이를 거래와 UTXO의 소유자의 주소를 대상으로 검증한 후, 검증에 성공할 시 1, 실패할 시 0을 반환한다.</br>
이외 더 복잡한 스크립트들도 적용할 수 있는데, 예를 들어 "X만큼 액면가를 지닌 OO코인을 나에게 보냈다는 SPV증명이 가능하다면 이 비트코인 UTXO 소유권을 얻을 수 있다"와 같은 것도 가능한데 이를 통해 탈중앙화 된 암호화폐 상호 교환이 가능하다.</br>
하지만 비트코인 상에서 구현된 스크립트 언어는 몇 가지 치명적인 단점이 있다.</br>
<li>튜링 완전 부재 : 비트코인 스크립팅 언어는 다양한 연산을 지원하지만 모든 연산이 가능한 것은 아니다. 대표적으로 루프 처리가 불가능한데, 이는 거래 검증 과정에서 무한루프를 피하기 위함이다. 이 방식은 비효율적인 스크립트를 만들어낸다.</li>
<li>Value-blindness : 하나의 UTXO 스크립트가 얼마를 인출할 수 있는지에 대한 세세한 제어를 제공하는 것이 불가능하다.</li>
<li>상태의 부재 : UTXO는 사용되거나 안되거나 둘중 하나의 상태만 가질 수 있기 때문에 다른 내부적인 상태를 가지도록 하는 멀티 스테이지 계약이나 스크립트를 만드는 것이 불가능하다.</li>
<li>Blockchain-blindness : UTXO는 논스, 타임스템프 등과 같은 정보를 담고 있지 않아서 잠재적으로 유용하게 쓰일 수 있는 랜덤성이라는 가치를 스크립팅 언어로부터 제외시키기 때문에 몇몇 카테고리에서 쓰임이 제한된다.</li>
