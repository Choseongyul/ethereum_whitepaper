...
이때 각 노드는 접두사를 이용해 구분하고 트리는 레벨DB에 저장한다. 위 그림에서 <p weight="bold">Extension Node</p>는 트라이의 길이를 압축하는 데 사용한다. 이 노드는 공통 접두사를 가지는 경로를 나타내고, 키의 일부를 나타내는 공통 접두사를 저장한다.</br>
키값이 길고, 중복되는 경로가 많을 때 이 중복을 제거하고, 트리의 특정 부분으로 빠르게 이동하기 위해 공통된 경로를 압축하여 저장하여 트리의 깊이를 줄여 검색 시간을 단축할 수 있음.</br>
<p weight="bold">Branch Node</p>는 트리 내에서 분기점을 나타낸다. 이 노드는 16진수 한 자리에 해당하는 16개의 포인터를 가질 수 있고, 트리의 각 분기를 나타낸다.</br>
